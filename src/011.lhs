...
There's quite a lot going on here so let's walk through it. We're taking each
index `i` in the grid, and then for each possible direction, we're calculating
the four adjacent indices gotten by starting at `i` and moving in that
direction – zero to three times. Finally we're just checking that each of the
indices is within our grid and extracting the actual product of the elements at
these positions.
