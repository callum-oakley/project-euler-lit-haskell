-- First we're going to need a function for calculating divisors. We'll just
-- check each potential divisor up to the square root, adding a pair of
-- divisors at a time (aside from when `n` is square).
